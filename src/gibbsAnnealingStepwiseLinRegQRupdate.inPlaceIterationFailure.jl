...
function gibbsStep(xCols, X, Y, xTestCols, Xtest, Ytest, lastR, currentCols, lastErrs, colsRecord, colsVec, T, iNum, Tsteps, startTime, acceptRate, dictHitRate, firstColsVec, printIter = true)
	...
	#change colsVec in place and use it instead of newColsVec
	colsVec[switchInd] = !colsVec[switchInd]
	
	errTimeStr = []

	t = time()
	(errs, Rnew) = if haskey(colsRecord, colsVec) # in(colsVec, map(a -> a[1], colsRecord))
		# colsRecord[find(a -> a == colsVec, map(a -> a[1], colsRecord))][1][2]
		#update newCols with entry from dictionary to ensure column order is preserved
		for i in 1:3 
			# println()
			push!(errTimeStr, "\n")
		...
		#modify currentCols in place based on if the switchInd was added or removed
		colsVec[switchInd] ? push!(currentCols, switchInd) : setdiff!(currentCols, switchInd)
		(colsRecord[colsVec][1], colsRecord[colsVec][2])
	else
		#otherwise form it from the previous value and the switching index
		# currentCols = if !colsVec[switchInd]
		# 	[currentCols; switchInd]
		# else
		# 	setdiff(currentCols, switchInd)
		# end
		...
		# println(string("Current Cols = ", currentCols))
		# println(string("New Cols = ", currentCols))
		# println(string("Switch Index = ", switchInd, " and has a value of ", colsVec[switchInd]))
		t2 = time()
		Rnew = if colsVec[switchInd]
			t3 = time()
		...
		# Xtmp = ones(eltype(xCols[1]), length(Y), length(currentCols)+1)
		# Xtmp2 = ones(eltype(xCols[1]), length(Ytest), length(currentCols)+1)
		#fill matrices with new cols, repeatedly using X and Xtest to avoid allocating new memory
		for j in eachindex(currentCols)
			if !colsVec[switchInd]
				X[:, j+1] = xCols[currentCols[j]]
			elseif j == length(currentCols) #if a column was added then only change X in the final added column since it was already formed above
				X[:, j+1] = xCols[currentCols[j]]
			end

			Xtest[:, j+1] = xTestCols[currentCols[j]]
		end
		# Xtmp = view(X, :, [1; currentCols+1])
		# Xtmp2 = view(Xtest, :, [1; currentCols+1])
		Xtmp = view(X, :, 1:length(currentCols)+1)
		Xtmp2 = view(Xtest, :, 1:length(currentCols)+1)
		tfill = time()-t2
		# println(string("New X fill time = ", tfill))
		push!(errTimeStr, string("New X fill time = ", tfill, "\n"))
		t2 = time()
	...
	if !acc
		#if change is rejected revert colsVec and currentCols
		# print("Reverting currentCols from $(currentCols)")
		if colsVec[switchInd] 
			setdiff!(currentCols, switchInd)
		else
			push!(currentCols, switchInd) 
		end
		# println(" to $(currentCols)")

		# println("Changing colsVec at index $(switchInd) to $(!colsVec[switchInd])")

		colsVec[switchInd] = !colsVec[switchInd] #if change is rejected revert colsVec
	end
	...
	# println(string("Converted New Cols Vec = ", find(colsVec)))
	# sleep(0.5)
	(errs, colsVec, changeString, acc, Rnew, currentCols)
...
function runGibbsStep(xCols, X, Y, Rnow, currentCols, BICsRecord, colsRecord, Tsteps, i = 1; startTime = time(), lastPrintTime = time(), acceptRate = 0, iterTime = 0, dictHitRate = 0, firstColsVec = fill(false, length(xCols)), printStep = true, updateInterval = 1.0)
	...
	if i == length(Tsteps)
		...
		runGibbsStep(xCols, X, Y, Rnew, newCols, newBICsRecord, push!(colsRecord, out[2] => dictEntry), Tsteps, i+1, startTime = startTime, lastPrintTime = newPrintTime, acceptRate = newAcceptRate, iterTime = newIterTime, dictHitRate = newDictHitRate, firstColsVec = firstColsVec, printStep = printStep, updateInterval = updateInterval)
	end
end
...
function runGibbsStep(xCols, X, Y, xTestCols, Xtest, Ytest, Rnow, currentCols, errsRecord, colsRecord, Tsteps, i = 1; startTime = time(), lastPrintTime = time(), acceptRate = 0, iterTime = 0, dictHitRate = 0, firstColsVec = fill(false, length(xCols)), printStep = true, updateInterval = 1.0)
	...
	if i == length(Tsteps)
		...
		newPrintTime = printIter ? stepStartTime : lastPrintTime
		runGibbsStep(xCols, X, Y, xTestCols, Xtest, Ytest, Rnew, newCols, newErrsRecord, push!(colsRecord, out[2] => dictEntry), Tsteps, i+1, startTime = startTime, lastPrintTime = newPrintTime, acceptRate = newAcceptRate, iterTime = newIterTime, dictHitRate = newDictHitRate, firstColsVec = firstColsVec, printStep = printStep, updateInterval = updateInterval)
	end
end

function runGibbsStep!(xCols, X, Y, xTestCols, Xtest, Ytest, Rnow, currentCols, errsRecord, colsRecord, Tsteps; firstColsVec = fill(false, length(xCols)), printStep = true, updateInterval = 1.0)
#modifies Rnow, currentCols, errsRecord, and colsRecord	of inputs in a loop instead of using recursion
	
	impliedColsVec = [in(a, currentCols) for a in eachindex(firstColsVec)]
	@assert impliedColsVec == firstColsVec "Initial columns do not match column vector"

	acceptRate = 0.0
	dictHitRate = 0.0
	iterTime = 0.0
	colsVec = copy(firstColsVec) #create a copy so we don't affect firstColsVec
	lastPrintTime = time()
	...
	for i in eachindex(Tsteps)
		# sleep(0.001)
		#gibbsStepInputs
		#gibbsStep(xCols, X, Y, xTestCols, Xtest, Ytest, lastR, currentCols, lastErrs, colsRecord, colsVec, T, iNum, Tsteps, startTime, acceptRate, dictHitRate, firstColsVec, printIter = true)
		...
		out = gibbsStep(xCols, X, Y, xTestCols, Xtest, Ytest, Rnow, currentCols, errsRecord[end][3], colsRecord, colsVec, Tsteps[i], i, Tsteps, iterTime, acceptRate, dictHitRate, firstColsVec, printIter)
		
		lastPrintTime = printIter ? lastPrintTime = time() : lastPrintTime

		# impliedColsVec = [in(a, currentCols) for a in eachindex(colsVec)]
		# @assert impliedColsVec == colsVec "Current columns $(currentCols) do not match column vector $(findall(colsVec))"


		if !haskey(colsRecord, out[2]) 
			push!(colsRecord, out[2] => (out[1], out[5], copy(out[6]))) #need to make a copy of currentCols because it gets modified)
		end

		if out[4]
			acceptRate = 0.95*acceptRate + 0.05
			Rnow = out[5]
			# println("Updating R with size of $(size(Rnow)) with currentCols $(currentCols)")
			# currentCols = out[6]
			push!(errsRecord, (out[3], copy(out[2]), out[1]))  #need to copy colsVec so the errRecord isn't affected in the future
			if !haskey(colsRecord, out[2]) 
				push!(colsRecord, out[2] => (out[1], out[5], copy(out[6]))) #need to make a copy of currentCols because it gets modified)
			end
		else
			# println("Keeping R with size of $(size(Rnow)) with currentCols $(currentCols)")
			acceptRate = 0.95*acceptRate
		end
		...
		dictHitRate = if haskey(colsRecord, out[2])
			dictHitRate*0.95 + 0.05
		else
			dictHitRate*0.95
		end
...
function runStepwiseAnneal(X::Array{T, 2}, Y::Array{T, 1}; initialColsVec = fill(false, size(X, 2)), initialColsRecord = Dict{Array{Bool, 1}, Tuple{Tuple{T, T}, Matrix{T}, Vector{Int64}}}(), iter = 0, printIter = true, updateInterval = 1.0) where T <: Real
	...
	Tmax = abs(BIC)/3000 #corresponds to a ~.1% increase in error having a ~1% chance of acceptance
